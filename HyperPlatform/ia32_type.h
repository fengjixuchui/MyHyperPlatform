...
union Ia32MtrrPhysBaseMsr {/// See: IA32_MTRR_PHYSBASEn and IA32_MTRR_PHYSMASKn Variable-Range Register Pair
  ULONG64 all;
  struct {
    ULONG64 type : 8;        //!< [0:7]
...
enum class memory_type : unsigned __int8 {/// See: Memory Types That Can Be Encoded With PAT Memory Types Recommended for VMCS and Related Data Structures
  kUncacheable = 0,
  kWriteCombining = 1,
  kWriteThrough = 4,
  kWriteProtected = 5,
...
struct VmControlStructure {/// See: Virtual-Machine Control Structures & FORMAT OF THE VMCS REGION
...
union VmxPinBasedControls {/// See: Definitions of Pin-Based VM-Execution Controls
  unsigned int all;
  struct {
...
union Ia32FeatureControlMsr {/// See: ARCHITECTURAL MSRS
...
union EptPointer {/// See: Extended-Page-Table Pointer (EPTP)
  ULONG64 all;
  struct {
