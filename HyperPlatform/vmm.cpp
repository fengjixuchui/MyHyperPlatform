...
_Use_decl_annotations_ static void VmmpHandleIoPort(GuestContext *guest_context)
{
    ...
    HYPERPLATFORM_LOG_DEBUG_SAFE("GuestIp= %016Ix, Port= %04x, %s%s%s", guest_context->ip, port, (is_in ? "IN" : "OUT"), (is_string ? "S" : ""), (is_string ? suffix : ""));
    ...
    if (is_string) {
        ...
        const auto update_register = (is_in) ? &guest_context->gp_regs->di : &guest_context->gp_regs->si;
